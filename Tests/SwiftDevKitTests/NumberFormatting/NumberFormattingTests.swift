...
struct NumberFormattingTests {
    ...
    @Test("Test compact notation formatting")
    ...
    func testUnitFormatting() throws {
        if #available(macOS 12.0, iOS 15.0, watchOS 8.0, tvOS 15.0, *) {
            ...
            // Length
            let meters = try distance.asUnit(UnitLength.meters, style: .medium, locale: nil)
            #expect(meters == "5.2 m")

            let kilometers = try distance.asUnit(UnitLength.kilometers, style: .medium, locale: nil)
            #expect(kilometers == "5.2 km")

            // Different styles
            let short = try distance.asUnit(UnitLength.kilometers, style: .short, locale: nil)
            #expect(short == "5.2km")

            let long = try distance.asUnit(UnitLength.kilometers, style: .long, locale: nil)
            #expect(long == "5.2 kilometers")

            // Different locale (German)
            let germanLocale = Locale(identifier: "de_DE")
            let germanFormat = try distance.asUnit(UnitLength.kilometers, style: .long, locale: germanLocale)
